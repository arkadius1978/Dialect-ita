...
const App: React.FC = () => {
  ...
  const connectToRoom = (rId: string, pName: string) => {
    ...
    getName((name) => {
      setGameState(prev => ({
        ...prev,
        playerNames: Array.from(new Set([...(prev.playerNames || []), name]))
      }));
    });
      ...
      <nav className="max-w-6xl mx-auto w-full px-4 mt-4">
        <div className="flex gap-4 border-b border-stone-300">
          {[
            ...
            { id: 'hand', label: `Mano (${gameState.hands[myPlayerName]?.length || 0})` },
            { id: 'discard', label: `Cimitero (${gameState.discardPile.length})` }
          ].map(tab => (
            <button 
              key={tab.id}
      ...
      <main className="max-w-6xl mx-auto w-full px-4 mt-8 flex-grow">
        ...
        {activeTab === 'discard' && (
          <div className="animate-in slide-in-from-left-4 duration-500">
            <h2 className="font-cinzel text-lg font-bold mb-6 text-stone-800 uppercase tracking-widest border-l-4 border-stone-500 pl-4">Cimitero (Carte Scartate)</h2>
            <div className="flex flex-wrap gap-6 justify-center md:justify-start">
              {gameState.discardPile.length === 0 ? (
                <div className="w-full text-center py-24 bg-stone-100 rounded-3xl border-2 border-dashed border-stone-200">
      ...
      <div className="max-w-md w-full bg-[#f5f2e9] rounded-[2rem] border-4 border-stone-800 shadow-2xl p-10 space-y-8 animate-in zoom-in-95">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input 
              placeholder="Es: Partita-Dialect" 
              value={rId}
